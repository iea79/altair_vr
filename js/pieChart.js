...
// pieChart('chart2', 'chartData2');

function pieChart(canvasEl, canvasLists) {

  // Config settings
  var chartSizePercent = 90;                        // The chart radius relative to the canvas width/height (in percent)
  var sliceBorderWidth = 0;                         // Width (in pixels) of the border around each slice
  var sliceBorderStyle = "#fff";                    // Colour of the border around each slice
  var sliceGradientColour = "rgba( 0, 0, 0, 0 )";                 // Colour to use for one end of the chart gradient
  var maxPullOutDistance = 10;                      // How far, in pixels, to pull slices out when clicked
  var pullOutFrameStep = 4;                         // How many pixels to move a slice with each animation frame
  var pullOutFrameInterval = 40;                    // How long (in ms) between each animation frame
  var pullOutLabelPadding = 0;                     // Padding between pulled-out slice and its label  
  var pullOutLabelFont = "bold 0 'Montserrat'";  // Pull-out slice label font
  var pullOutValueFont = "bold 0 'Montserrat'";  // Pull-out slice value font
  var pullOutValuePrefix = "$";                     // Pull-out slice value prefix
  var pullOutShadowColour = "rgba( 0, 0, 0, 0 )";  // Colour to use for the pull-out slice shadow
  ...
  var pullOutBorderWidth = 0;                       // Width (in pixels) of the pull-out slice border
  var pullOutBorderStyle = "#fff";                  // Colour of the pull-out slice border
  ...
  function init() {
    ...
    console.log(canvas)
    var canvasTable = document.getElementById(canvasLists);
    console.log(canvasTable)
    // var canvasList2 = document.getElementById('chartData2');
    // var canvasList = canvas.closest('.grid__wrapper').find('.chartData');
